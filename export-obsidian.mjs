...
function buildAuthorLink(author) {
  const label = author || 'Unknown';
  return `[[Dashboards/Authors#${label}|${label}]]`;
}

function buildTagLink(tag) {
  const label = tag || 'Untagged';
  return `[[Dashboards/Tags#${label}|${label}]]`;
}

function buildIssueNote(issue, pages, stats) {
  const authorLinks = normalizeListField(issue.authors).map((author) => buildAuthorLink(author));
  const tagLinks = normalizeListField(issue.tags).map((tag) => buildTagLink(tag));
  const lines = [
    ...
    `- authors: ${authorLinks.join(', ') || 'Unknown'}`,
    `- tags: ${tagLinks.join(', ') || 'Untagged'}`,
